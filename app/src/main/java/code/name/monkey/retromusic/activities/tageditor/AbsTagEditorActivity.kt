...
abstract class AbsTagEditorActivity<VB : ViewBinding> : AbsBaseActivity() {
    ...
    internal val albumArtist: String?
        get() {
            return try {
                getAudioFile(songPaths!![0]).tagOrCreateAndSetDefault.getAll(FieldKey.ALBUM_ARTIST).joinToString(", ")
            } catch (e: Exception) {
                logE(e)
                null
            }
    ...
    protected val composer: String?
        get() {
            return try {
                getAudioFile(songPaths!![0]).tagOrCreateAndSetDefault.getAll(FieldKey.COMPOSER).joinToString(", ")
            } catch (e: Exception) {
                logE(e)
                null
            }
    ...
    protected val artistName: String?
        get() {
            return try {
                getAudioFile(songPaths!![0]).tagOrCreateAndSetDefault.getAll(FieldKey.ARTIST).joinToString(", ")
            } catch (e: Exception) {
                logE(e)
                null
            }
